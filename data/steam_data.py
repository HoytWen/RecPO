...
def clean_u_prefixes(code):
    # Regex to target 'u' prefixes and handle edge cases
    pattern = re.compile(r'''
        \b                      # Word boundary
        (                       # Group 1: Prefix (capture 'u's and other modifiers)
            (?:                 # Non-capturing group for allowed prefixes
                [rRfFbB]*       # Any combination of r/f/b (case-insensitive)
                u+              # One or more 'u's (case-insensitive)
                | u+            # Or standalone 'u's
            )
        )
        (                       # Group 2: Quotes (single/double/triple)
            (?:'{1,3}|"{1,3})
        )
    ''', re.VERBOSE)
    ...
    def replacer(match):
        ...
        # Remove all 'u's from the prefix, preserving other characters (e.g., 'r', 'f', 'b')
        new_prefix = re.sub(r'[uU]', '', prefix)
        return f'{new_prefix}{quotes}'
...
class SteamData_rating(data.Dataset):
    ...
    def get_game_id2name(self):
        raw_path = op.join(self.data_dir, 'Steam')
        item_path = op.join(raw_path, 'steam_new.item')
        game_token2name = dict()
        with open(item_path, 'r', encoding="ISO-8859-1") as f:
            for l in f.readlines()[1:]:
                ...
                game_token2name[ll[4]] = title

        game_id2name = dict()
        token2id_path = op.join(raw_path, 'token2id.json')
        token2id_path = op.join(token2id_path)
        with open(token2id_path, "r") as f:
            token2id = json.load(f)
            for token, idx in token2id.items():
                if token == '[PAD]':
                    continue
                title = game_token2name[token]
                title = clean_u_prefixes(title).strip('"')
                title = title.encode('latin-1').decode('unicode_escape')
                game_id2name[idx] = title
